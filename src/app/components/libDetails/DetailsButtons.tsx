...
const DetailsButtons = ({ data }: DetailsButtonsProps) => {
  ...
  return (
    <>
      <button onClick={() => handleMyPlan(data)} className="cursor-pointer flex justify-start items-center gap-2.5 text-[#0F1115] bg-[#CCFF00] py-2.5  px-5.5 rounded-lg font-semibold text-[14px]">
        <CiCalendarDate />
        Add to today's plan
      </button>
      <button onClick={() => handleSaved(data)} className="cursor-pointer flex justify-start items-center gap-2.5 border border-[#374151] text-[#E5E7EB]  py-2.5  px-5.5 rounded-lg font-medium text-[14px]">
        <FaRegBookmark />
        Save for later
      </button>
    </>
